...
if { $impl_flow == "CLOUD_BASE" } {
  set use_xpr_setting TRUE
  set prefix base_
elseif { $impl_flow == "CLOUD_MERGE" } {
  set framework_setting FALSE
  set prefix merge_
else 
  set framework_setting TRUE
  set prefix
}

##
## optimizing design
if { $use_xpr_setting == "FALSE" } {
...
report_utilization    -quiet -file  $rpt_dir/${prefix}utilization_route_design.rpt
report_route_status   -quiet -file  $rpt_dir/${prefix}route_status.rpt
report_timing_summary -quiet -max_paths 100 -file $rpt_dir/${prefix}timing_summary.rpt
report_drc            -quiet -ruledeck bitstream_checks -name psl_fpga -file $rpt_dir/${prefix}drc_bitstream_checks.rpt


##
## checking timing
## Extract timing information, change ns to ps, remove leading 0's in number to avoid treatment as octal.
set TIMING_WNS [exec grep -A6 "Design Timing Summary" $rpt_dir/${prefix}timing_summary.rpt | tail -n 1 | tr -s " " | cut -d " " -f 2 | tr -d "." | sed {s/^\(\-*\)0*\([1-9]*[0-9]\)/\1\2/}]
