...
#     step "$SNAP_ROOT/software/examples/snap_memcopy -C0 -i ../../1KB.txt -o 1KB.out -t10"
      ...
      # for size in 1 2 4 8 16 32 64; do
        for size in 1 64 80 85 240 $rnd1k $rnd1k4k; do   # 64B aligned       01/20/2017: error 128B issues 120, CR968181, wait for Vivado 2017.1
        ...
        to=$((size*50+10))                                     # rough timeout dependent on filesize
...
#     step "$SNAP_ROOT/software/examples/snap_search -p'A' -C0 -i ../../1KB.txt   -t100      "
      ...
      # for size in {1..5}; do
        ...
        # dd if=/dev/urandom bs=${size} count=1 >${size}.in;                                            # random data any char, no echo due to unprintable char
          cmd='print("A" * '${size}', end="")'; python3 -c "$cmd" >${size}.uni;                         # uniform data generated with python
