...
// start with groovy
  ...
  def XIL    = ['2015.4'   ,'2016.3'   ,'2015.4'      ] // Xilinx Vivado version
  def IES    = ['14.10.s14','15.10.s19','15.10.s19'   ] // Cadence IES versions
  def CRD    = ['KU3'      ,'KU3'      ,'GT'          ] // HDK versions
  def ACT    = ['1'        ,'1'        ,'2'           ] // Action example
//def ACT    = ['memcopy'  ,'memcopy'  ,'opencldesign'] // path to action
  def DDR    = ['TRUE'     ,'FALSE'    ,'FALSE'       ] // connect DDR interface
  def execute1 = [:]
  for(int i = 0; i < XIL.size(); i++) {
    if (enable[i] == "1") {
      execute1[i] = create_model(enable[i],XIL[i],IES[i],CRD[i],ACT[i],DDR[i])
    }
  }
  parallel execute1

  def create_model(String ena,String XIL,String IES,String CRD,String ACT,String DDR) {
    cmd={
      echo "arguments; ena=${ena} XIL=${XIL} IES=${IES} CRD=${CRD} ACT=${ACT} DDR=${DDR}"
      echo "jenkins url =${env.JENKINS_URL} build_url=${env.BUILD_URL}"
      echo "branch=${env.BRANCH_NAME} build id=${env.BUILD_ID} number=${env.BUILD_NUMBER} tag=${env.BUILD_TAG}"
      ...
      // Jenkins variables for this one create_model_function
            ...
            dir('donut/hardware/setup') {
              ...
              withEnv(["XILINX_VIVADO=/afs/bb/proj/fpga/xilinx/Vivado/${XIL}",
                ...
                sh """                                        # different comments in shell
                  ...
                  vivado -version
                  SIMULATOR=irun make config model
                """
              } // end withEnv
            } // end dir
          } // end stage config
          ...
          stage("simulate irun"){
            dir("donut/hardware/sim") {
              withEnv(["XILINX_VIVADO=/afs/bb/proj/fpga/xilinx/Vivado/${XIL}",
                ...
                sh """                                        # different comments in shell
                  source \$XILINX_VIVADO/settings64.sh        # to be sourced in each shell again
                  SIMULATOR=ncsim ./run_sim -app tools/stage2 -a2 -v -t100   // branch=jenkinstest always builds 3step
                """
              } // end withEnv
            } // end dir
          } // end stage
