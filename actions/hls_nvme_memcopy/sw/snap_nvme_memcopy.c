...
static void usage(const char *prog)
{
	printf("Usage: %s [-h] [-v, --verbose] [-V, --version]\n"
	   ...
	   " Example on a real FPGA:\n"
	   " -----------------------\n"
           " Before using NVME following commands must be run :\n"
	   " echo Discover mechanism to find the action attached to the card in slot n:\n"
	   "  snap_maint -vv -Cn\n"
	   " echo Initialize the NVME memory driver:\n"
	   "  snap_nvme_init\n"
	   "\n"
	   " echo create a 512MB file with random data ...wait few minutes!..\n"
	   "  dd if=/dev/urandom of=in.bin bs=1M count=512\n"
           "\n"
	   " echo move in.bin data from Host DRAM to @123 in NVME and then back to Host DRAM (result in out.bin)\n"
           "  snap_nvme_memcopy -A HOST_DRAM -D NVME_SSD -i in.bin -d 0x123\n"
           "  snap_nvme_memcopy -A NVME_SSD -D HOST_DRAM -a 0x123 -o out.bin -s 0x20000000\n"
           "  diff in.bin out.bin\n"
	   "\n"
	   " echo by default DDR buffer_in is at @0 and DDR buffer_out is at 0x80000000\n"
	   " echo Checking : Read DDR buffer_in at @0 (default) -result in buf_in.bin\n"
	   " echo Checking : Read DDR buffer_out at @0x80000000 (default) -result in buf_out.bin\n"
           "  snap_nvme_memcopy -A CARD_DRAM -D HOST_DRAM -o buf_in.bin -s 128 -a 0x0\n"
           "  diff in.bin buf_in.bin\n"
           "  snap_nvme_memcopy -A CARD_DRAM -D HOST_DRAM -o buf_out.bin -s 128 -a 0x80000000\n"
           "  diff in.bin buf_out.bin\n"
	   "\n"
	   " echo move in.bin data from Host DRAM to FPGA DDR, then to NVME and then back to DDR and to Host DRAM (result in out2.bin)\n"
           "  snap_nvme_memcopy -A HOST_DRAM -D CARD_DRAM -i in.bin -d 0x00000000\n"
           "  snap_nvme_memcopy -A CARD_DRAM -D NVME_SSD  -a 0x00000000 -d 0x00000000 -s 0x20000000\n"
           "  snap_nvme_memcopy -A NVME_SSD  -D CARD_DRAM -a 0x00000000 -d 0x80000000 -s 0x20000000\n"
           "  snap_nvme_memcopy -A CARD_DRAM -D HOST_DRAM -a 0x80000000 -o out2.bin -s 0x20000000\n"
           "  diff in.bin out2.bin\n"
           "\n"
	   ...
	   " echo Discover mechanism to find the action attached to the card in slot n:\n"
	   "  snap_maint -vv -Cn\n"
	   " echo Initialize the NVME memory driver (ONLY if using Denali models):\n"
	   "  snap_nvme_init\n"
	   "\n"
           " echo create a 128B file with random data ...\n"
           "  dd if=/dev/urandom of=in.bin bs=1 count=128\n"
           "\n"
	   " echo move in.bin data from Host DRAM to @0x123 in NVME and then read back to Host DRAM (result in out.bin)\n"
           "  snap_nvme_memcopy -A HOST_DRAM -D NVME_SSD  -i in.bin -d 0x123\n"
           "  snap_nvme_memcopy -A NVME_SSD -D HOST_DRAM -a 0x123 -o out.bin -s 128\n"
           "  diff in.bin out.bin\n"
           "\n"
	   " echo by default DDR buffer_in is at @0 and DDR buffer_out is at 0x80000000\n"
	   " echo Checking : Read DDR buffer_in at @0 (default) -result in buf_in.bin\n"
