...
static void usage(const char *prog)
{
	printf("Usage: %s [-h] [-v, --verbose] [-V, --version]\n"
	   ...
	   "\n"
	   " echo move in.bin data from Host DRAM to FPGA memory and back to Host DRAM (result in out.bin)\n"
           "  snap_nvme_memcopy -A HOST_DRAM -D HOST_DRAM -i in.bin -o out1.bin\n"
	   "\n"
	   " echo move in.bin data from Host DRAM to FPGA DDR and then back to Host DRAM (result in out.bin)\n"
           "  snap_nvme_memcopy -A HOST_DRAM -D CARD_DRAM -i in.bin -d 0xD000\n"
           "  snap_nvme_memcopy -A CARD_DRAM -D HOST_DRAM -a 0xD000 -o out2.bin -s 0x20000000\n"
           "\n"
	   " echo move in.bin data from Host DRAM to NVME and then back to Host DRAM (result in out.bin)\n"
           "  snap_nvme_memcopy -A HOST_DRAM -D NVME_SSD  -i in.bin -d 0xE000\n"
           "  snap_nvme_memcopy -A NVME_SSD -D HOST_DRAM -a 0xE000 -o out3.bin -s 0x20000000\n"
           "\n"
	   " echo move in.bin data from Host DRAM to FPGA DDR, then to NVME and then back to DDR and to Host DRAM (result in out.bin)\n"
           "  snap_nvme_memcopy -A HOST_DRAM -D CARD_DRAM -i in.bin -d 0xD000\n"
           "  snap_nvme_memcopy -A CARD_DRAM -D NVME_SSD  -a 0xD000 -d 0xE000 -s 0x20000000\n"
           "  snap_nvme_memcopy -A NVME_SSD  -D CARD_DRAM -a 0xE000 -d 0xD000 -s 0x20000000\n"
           "  snap_nvme_memcopy -A CARD_DRAM -D HOST_DRAM -a 0xD000 -o out4.bin -s 0x20000000\n"
           "\n"
	   ...
	   " echo move in.bin data from Host DRAM to FPGA DDR and then back to Host DRAM (result in out.bin)\n"
           "  snap_nvme_memcopy -A HOST_DRAM -D CARD_DRAM -i in.bin -d 0xD000\n"
           "  snap_nvme_memcopy -A CARD_DRAM -D HOST_DRAM -a 0xD000 -o out2.bin -s 128\n"
           "\n"
	   " echo move in.bin data from Host DRAM to NVME and then back to Host DRAM (result in out.bin)\n"
           "  snap_nvme_memcopy -A HOST_DRAM -D NVME_SSD  -i in.bin -d 0xE000\n"
           "  snap_nvme_memcopy -A NVME_SSD -D HOST_DRAM -a 0xE000 -o out3.bin -s 128\n"
           "\n"
