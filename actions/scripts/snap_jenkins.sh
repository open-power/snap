...
function test_hard()
{
	...
	while [ 1 ]; do
		wait_flag=0
		if [[ $accel != "AD9V3" ]] && [[ $accel != "RCXVUP" ]] && [[ $accel != "AD9H3" ]] && [[ $accel != "AD9H7" ]]; then
...
function usage() {
	...
	echo "    1.) Option -A [N250S, N250SP, ADKU3, AD8K5, S121B, FX609, S241], AD9V3, AD9H3, AD9H7 or RCXVUP] and -F is set"
	echo "    		[AD9V3, AD9H3, AD9H7  or RCXVUP]  and -F as well as -f are set (Their 2 SPI flashes require 2 bin files)"
	...
	echo "    2.) Option -A [N250S, N250SP, ADKU3, AD8K5, S121B, FX609, S241, AD9V3, AD9H3, AD9H7 or RCXVUP]"
...
# version 1.2 adds support for AD9H3 card
# version 1.3 adds support for AD9H7 card
VERSION=1.3
# --------------------------------------------------------
PROGRAM=$0
BINFILE=""
	...
	A)
		...
		if [[ $accel != "N250S"  ]] &&
		   ...
		   [[ $accel != "ALL"    ]]; then
			...
			echo "Expect: [N250S N250SP ADKU3 AD8K5 S121B FX609 S241 RCXVUP AD9V3 AD9H3 AD9H7 or ALL]" >&2
			exit 1
		fi
		;;
	F)
...
for card in $MY_CARDS ; do
	...
	if [[ $accel != "N250S" ]]  && [[ $accel != "N250SP" ]] && [[ $accel != "ADKU3" ]] && [[ $accel != "S121B" ]] && [[ $accel != "AD9V3" ]] && [[ $accel != "S241" ]] && [[ $accel != "FX609" ]] && [[ $accel != "RCXVUP" ]] && [[ $accel != "AD9H3" ]] && [[ $accel != "AD9H3" ]]; then
