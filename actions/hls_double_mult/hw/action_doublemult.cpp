...
static void mbus_to_double(snap_membus_t *data_read, double *table_double_in)
{
	union {
		uint64_t value_u;
		double   value_d;
...
static void  double_to_mbus(double *table_double_out, snap_membus_t *data_to_be_written)
{
	union {
		double   value_d;
		uint64_t value_u;
			  ...
			  action_reg *act_reg)
{
	...
	double data_in[MAX_NB_OF_DOUBLES_READ], data_out[MAX_NB_OF_DOUBLES_READ];
	...
	memcpy(buffer_in, (snap_membus_t*)(din_gmem + i_idx), MAX_NB_OF_DOUBLES_READ*sizeof(double));
