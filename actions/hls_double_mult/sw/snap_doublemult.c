...
//	uint8_t trailing_zeros[1024] = { 0, };
	...
	action_datatype_t double1 = 0, double2 = 0, double3 = 0, result[128];
	action_datatype_t *data_in = NULL, *data_out = NULL;
	...
	data_in = (action_datatype_t*)snap_malloc(128 * sizeof(action_datatype_t));
	if (data_in == NULL)
		goto out_error;
	...
	for(int i = 0; i < size ; i++){
		data_in[i] = (action_datatype_t)(2.1f);// + (action_datatype_t)(i*1.0f); //2.1354;//1 + 0.5*i;
//		fprintf(stdout,"data_in[%d] = %f\n",i, (float)(*(data_in+i)));
	}
	...
	fprintf(stdout, "Preparing data:\n");
	fprintf(stdout, "double1 = %f, double2 = %f double3 = %f\n", (float)double1, (float)double2, (float)double3);

    // Process data multiplying double 3 by 3
     for ( int i = 0; i < size/3; i++ ) {
     	if(i == 0) result[i] = data_in[i]+(action_datatype_t)1.2f;//(action_datatype_t)1.0 * data_in[3*i] * data_in[(3*i)+1]; //* data_in[(3*i)+2];
     	else
     		result[i] = data_in[i]*(i + i*(action_datatype_t)0.1f);
     }
	...
	data_out = (action_datatype_t*)snap_malloc(128 * sizeof(action_datatype_t));
	if (data_out == NULL)
		goto out_error;
	// write 0 into this buffer
...
//        }
	...
	fprintf(stdout, "Host Result = %lf, Action Result = %f\n", (float)result[0], (float)*data_out);
	fprintf(stdout, "All products results from action are below \n");
	fprintf(stdout, "A = %lf & B = %lf & C = %lf  ", (float)data_out[0],  (float)data_out[1], (float)data_out[2]);
	fprintf(stdout, "D = %lf & E = %lf\n", (float)data_out[3],  (float)data_out[4]);
	fprintf(stdout, "All products results expected are below \n");
	fprintf(stdout, "A = %lf & B = %lf & C = %lf  ", (float)*(result),  (float)*(result + 1), (float)*(result + 2));
