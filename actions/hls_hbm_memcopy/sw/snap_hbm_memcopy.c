...
static void usage(const char *prog)
{
	printf("Usage: %s [-h] [-v, --verbose] [-V, --version]\n"
	       ...
	       "snap_maint -vv -C0\n"
	       "echo create a 256MB file with random data ...wait...\n"
	       "dd if=/dev/urandom of=t1 bs=1M count=256\n"
	       "\n"
	       "echo READ 256MB from Host - one direction\n"
	       "snap_hbm_memcopy -i t1\n"
	       "echo WRITE 256MB to Host - one direction - (t1!=t2 since buffer is 256KB)\n"
	       "snap_hbm_memcopy -o t2 -s0x10000000\n"
	       "\n"
	       "echo READ 256MB from HBM port0- one direction\n"
	       "snap_hbm_memcopy -A HBM_P0 -a0x0 -s0x10000000\n"
	       "echo WRITE 512MB to HBM port0 - one direction\n"
	       "snap_hbm_memcopy -D HBM_P0 -d0x0 -s0x10000000 \n"
	       "\n"
	       "echo MOVE 256MB from Host to HBM port0 -> port0 to port 1 and back to Host and compare\n"
	       "snap_hbm_memcopy -i t1 -D HBM_P0 -d 0x0 \n"
	       "snap_hbm_memcopy -A HBM_P0 -a 0x0 -D HBM_P1 -d 0x0 -s0x10000000 \n"
	       "snap_hbm_memcopy -A HBM_P1 -a 0x0 -o t2 -s0x10000000 \n"
	       "diff t1 t2\n"
	       "\n"
