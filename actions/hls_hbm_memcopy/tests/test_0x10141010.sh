...
function test_memcopy {
    ...
    echo -n "Doing snap_hbm_memcopy (aligned) ${size} bytes ... "
    cmd="snap_hbm_memcopy -C${snap_card} -X -N \
		-i ${size}_A.bin	\
		-o ${size}_A.out >>	\
...
function test_memcopy_to_ddr {
    ...
    cmd="snap_hbm_memcopy -C${snap_card} -X -N \
		-i ${size}_A.bin	\
                -d 0x0 -D HBM_P0 >>  \
		snap_hbm_memcopy_to_ddr.log 2>&1"
    eval ${cmd}
    if [ $? -ne 0 ]; then
	cat snap_hbm_memcopy_to_ddr.log
	echo "cmd: ${cmd}"
	echo "failed"
	exit 1
    fi
...
rm -f snap_hbm_memcopy_to_ddr.log
touch snap_hbm_memcopy_to_ddr.log
...
echo
echo "WRITE to FPGA HBM_P0 Performance Results"
grep "memcopy of" snap_hbm_memcopy_to_ddr.log
echo

#### MEMCOPY from FPGA HBM PORT 0 #####################################
...
function test_memcopy_from_ddr {
    ...
    echo -n "Doing snap_hbm_memcopy (aligned) ${size} bytes ... "
    cmd="snap_hbm_memcopy -C${snap_card} -X	\
		-o ${size}_A.out	\
                -a 0x0 -A HBM_P0 -s ${size} >>  \
		snap_hbm_memcopy_from_ddr.log 2>&1"
    eval ${cmd}
    if [ $? -ne 0 ]; then
	cat snap_hbm_memcopy_from_ddr.log
	echo "cmd: ${cmd}"
	echo "failed"
	exit 1
    fi
...
rm -f snap_hbm_memcopy_from_ddr.log
touch snap_hbm_memcopy_from_ddr.log
...
echo
echo "READ from FPGA HBM Port 0 Performance Results"
grep "memcopy of" snap_hbm_memcopy_from_ddr.log
echo
...
echo -n "Doing snap_hbm_memcopy (HBM_P0) ${size} bytes to card ... "
cmd="snap_hbm_memcopy -C${snap_card} -N		\
		-i $test_data -D HBM_P0 -d 0x00000000 >	\
		snap_hbm_memcopy_card.log 2>&1"
eval ${cmd}
if [ $? -ne 0 ]; then
    cat snap_hbm_memcopy_card.log
    echo "cmd: ${cmd}"
    echo "failed"
    exit 1
fi
...
echo -n "Doing snap_hbm_memcopy (HBM_P0) ${size} bytes from card... "
cmd="snap_hbm_memcopy -C${snap_card} -N	\
		-A HBM_P0 -a 0x00000000 -s ${size}	\
		-o snap_search.out >>		\
		snap_hbm_memcopy_card.log 2>&1"
eval ${cmd}
if [ $? -ne 0 ]; then
