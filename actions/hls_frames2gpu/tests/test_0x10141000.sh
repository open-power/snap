...
function test_frames2gpu {
    local size=$1

    dd if=/dev/urandom of=${size}_A.bin count=1 bs=${size} 2> dd.log

    echo -n "Doing snap_frames2gpu (aligned) ${size} bytes ... "
    cmd="snap_frames2gpu -C${snap_card} -X -N \
		-i ${size}_A.bin	\
		-o ${size}_A.out >>	\
		snap_frames2gpu.log 2>&1"
    eval ${cmd}
    if [ $? -ne 0 ]; then
	cat snap_frames2gpu.log
	echo "cmd: ${cmd}"
	echo "failed"
	exit 1
    fi
...
rm -f snap_frames2gpu.log
touch snap_frames2gpu.log

if [ "$duration" = "SHORT" ]; then
    for (( size=64; size<10000; size*=2 )); do
	test_frames2gpu ${size}
    done
fi
...
grep "frames2gpu of" snap_memcopy.log
echo

#### MEMCOPY to CARD DDR ##############################################

function test_frames2gpu_to_ddr {
    local size=$1
    ...
    cmd="snap_frames2gpu -C${snap_card} -X -N \
		-i ${size}_A.bin	\
                -d 0x0 -D CARD_DRAM >>  \
		snap_frames2gpu_to_ddr.log 2>&1"
    eval ${cmd}
    if [ $? -ne 0 ]; then
	cat snap_frames2gpu_to_ddr.log
	echo "cmd: ${cmd}"
	echo "failed"
	exit 1
    fi
...
rm -f snap_frames2gpu_to_ddr.log
touch snap_frames2gpu_to_ddr.log

if [ "$duration" = "SHORT" ]; then
    for (( size=64; size<10000; size*=2 )); do
	test_frames2gpu_to_ddr ${size}
    done
fi
...
grep "frames2gpu of" snap_memcopy_to_ddr.log
echo

#### MEMCOPY from CARD DDR ############################################

function test_frames2gpu_from_ddr {
    local size=$1

    dd if=/dev/urandom of=${size}_A.bin count=1 bs=${size} 2> dd.log

    echo -n "Doing snap_frames2gpu (aligned) ${size} bytes ... "
    cmd="snap_frames2gpu -C${snap_card} -X	\
		-o ${size}_A.out	\
                -a 0x0 -A CARD_DRAM -s ${size} >>  \
		snap_frames2gpu_from_ddr.log 2>&1"
    eval ${cmd}
    if [ $? -ne 0 ]; then
	cat snap_frames2gpu_from_ddr.log
	echo "cmd: ${cmd}"
	echo "failed"
	exit 1
    fi
...
rm -f snap_frames2gpu_from_ddr.log
touch snap_frames2gpu_from_ddr.log

if [ "$duration" = "SHORT" ]; then
    for (( size=64; size<10000; size*=2 )); do
	test_frames2gpu_from_ddr ${size}
    done
fi
...
grep "frames2gpu of" snap_memcopy_from_ddr.log
echo
...
echo -n "Doing snap_frames2gpu (CARD_DRAM) ${size} bytes to card ... "
cmd="snap_frames2gpu -C${snap_card} -N		\
		-i $test_data -D CARD_DRAM -d 0x00000000 >	\
		snap_frames2gpu_card.log 2>&1"
eval ${cmd}
if [ $? -ne 0 ]; then
    cat snap_frames2gpu_card.log
    echo "cmd: ${cmd}"
    echo "failed"
    exit 1
fi
...
echo -n "Doing snap_frames2gpu (CARD_DRAM) ${size} bytes from card... "
cmd="snap_frames2gpu -C${snap_card} -N	\
