				 ...
				 snapu64_t size_in_bytes_to_transfer)
{
	...
	case SNAP_ADDRTYPE_CARD_DRAM2:
		// Patch to the issue#652 - memcopy doesn't handle small packets
		//memcpy((snap_membus_t  *) (d_ddrmem + output_address),
		//       buffer, size_in_bytes_to_transfer);

		// Do not insert anything more in this loop to not break the burst
		wb_ddr_loop2: for (int k=0; k<size_in_words; k++)
		#pragma HLS PIPELINE
                    (d_ddrmem2 + output_address)[k] = buffer[k];
		// end of patch
				  ...
				  snapu64_t size_in_bytes_to_transfer)
{
	...
	case SNAP_ADDRTYPE_CARD_DRAM2:
		memcpy(buffer, (snap_membus_t  *) (d_ddrmem2 + input_address),
		       size_in_bytes_to_transfer);
       		rc =  0;
		break;
...
#pragma HLS UNROLL		// cannot completely unroll a loop with a variable trip count
		...
		rc |= read_burst_of_data_from_mem(din_gmem, d_ddrmem, d_ddrmem2,
			act_reg->Data.in.type,
			InputAddress + address_xfer_offset, buf_gmem, xfer_size);

		rc |= write_burst_of_data_to_mem(dout_gmem, d_ddrmem, d_ddrmem2,
...
#pragma HLS INTERFACE m_axi port=d_ddrmem2 bundle=card_mem1 offset=slave depth=512 \
  max_read_burst_length=64  max_write_burst_length=64
#pragma HLS INTERFACE s_axilite port=d_ddrmem2 bundle=ctrl_reg offset=0x060
...
#pragma HLS INTERFACE s_axilite port=return bundle=ctrl_reg
	...
	case 0:
		...
		break;
	default:
        	process_action(din_gmem, dout_gmem, d_ddrmem, d_ddrmem2, act_reg);
		break;
	}
}
