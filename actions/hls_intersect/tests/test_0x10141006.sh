...
#### INTERSECT SORT  #########################################################
echo "Doing snap_intersect (Sort  method) ... "
rm -f snap_intersect_s.log
touch snap_intersect_s.log

for table_num in 10 23 256 512 1024 2048 4096 8192 16384 32768 65536 131072 262144 524288 1048576 2097152; do
#for table_num in 10 23 256 512 1024 2048 4096 8192 16384 32768 65536 131072 262144 524288 1048576 2097152 4194304 8388608 ; do
   ...
   #gen_cmd="gen_input_table2.pl $table_num $table_num 50";
    echo "$gen_cmd" >> snap_intersect_s.log
    eval ${gen_cmd}
   
    cmd="snap_intersect -C${snap_card} -i table1.txt -j table2.txt -m2 \
			>> snap_intersect_s.log 2>&1"
    echo "$cmd" >> snap_intersect_s.log
    eval ${cmd}

    cmd="snap_intersect -C${snap_card} -i table1.txt -j table2.txt -m2 -s \
			>> snap_intersect_s.log 2>&1"
    echo "$cmd" >> snap_intersect_s.log
    eval ${cmd}

    if [ $? -ne 0 ]; then
	cat snap_intersect_s.log
	echo
	echo "cmd: ${cmd}"
	echo "failed"
	exit 1
...
cmd="process.awk snap_intersect_s.log"
eval ${cmd}
if [ $? -ne 0 ]; then
   echo "failed"
   exit 1
