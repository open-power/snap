...
if [ $search -eq 1 ]; then
	...
	cmd="./examples/snap_search -C${snap_card} -E 1024	\
		-i examples/1KiB_X.bin -p X >			\
		examples/snap_search2.log 2>&1"
	eval ${cmd}
	if [ $? -ne 0 ]; then
		cat examples/snap_search2.log
		echo "cmd: ${cmd}"
		echo "failed"
		exit 1
	fi
	...
	grep '1024 patterns' ./examples/snap_search2.log 2>&1 > /dev/null || exit 1
	if [ $? -ne 0 ]; then
		echo "failed"
		exit 1
	fi
	...
	cmd="./examples/snap_search -C${snap_card} -E 2048	\
		-i examples/3KiB.bin -p X > 			\
		examples/snap_search3.log 2>&1"
	eval ${cmd}
	if [ $? -ne 0 ]; then
		cat examples/snap_search3.log
		echo "cmd: ${cmd}"
		echo "failed"
		exit 1
	fi
	...
	echo -n "Check results ... "
	grep '2048 patterns' ./examples/snap_search3.log 2>&1 > /dev/null || exit 1
	if [ $? -ne 0 ]; then
		echo "failed"
		exit 1
	fi
...
if [ $memcopy -eq 1 ]; then
	...
	cmd="./examples/snap_memcopy -C${snap_card} -X	\
		-i examples/1KiB_A.bin			\
		-o examples/1KiB_A.out >		\
		examples/snap_memcopy.log 2>&1"
	eval ${cmd}
	if [ $? -ne 0 ]; then
		cat examples/snap_memcopy.log
		echo "cmd: ${cmd}"
		echo "failed"
		exit 1
	fi
...
if [ $memcopy -eq 1 -a $memcopy_cardram -eq 1 ]; then
	...
	# examples/snap_search.txt

	size=`ls -l $test_data | cut -d' ' -f5`

	echo -n "Doing snap_memcopy (CARD_DRAM) ${size} bytes to card ... "
	cmd="./examples/snap_memcopy -C${snap_card}		\
		-i $test_data -D CARD_DRAM -d 0x00000000 >	\
		examples/snap_memcopy_card.log 2>&1"
	eval ${cmd}
	if [ $? -ne 0 ]; then
		cat examples/snap_memcopy_card.log
		echo "cmd: ${cmd}"
		echo "failed"
		exit 1
	fi
	...
	echo -n "Check results ... "
	diff $test_data examples/snap_search.out 2>&1 > /dev/null
	if [ $? -ne 0 ]; then
		echo "failed"
		echo "  examples/snap_search.txt examples/snap_search.out are different!"
		exit 1
	fi
	echo "ok"
fi
...
if [ $memcopy -eq 1 -a $memcopy_unaligned -eq 1 ]; then
	...
	echo -n "Check results ... "
	diff examples/snap_search.txt examples/snap_search.out 2>&1 > /dev/null
	if [ $? -ne 0 ]; then
		echo "failed"
		echo "  examples/snap_search.txt examples/snap_search.out are different!"
		exit 1
...
if [ $hashjoin -eq 1 ]; then
	...
	rm -f examples/snap_hashjoin.log
	touch examples/snap_hashjoin.log
	for t2_entries in `seq 1 128` 512 666 888 999 1024 2048 2049 5015 7007 8088 123123 ; do
		echo -n "  ${t2_entries} entries for T2 ... "
		cmd="./examples/snap_hashjoin -C${snap_card} -T ${t2_entries} -v \
			>> examples/snap_hashjoin.log 2>&1"
		echo "$cmd" >> examples/snap_hashjoin.log
		eval ${cmd}
		if [ $? -ne 0 ]; then
			cat examples/snap_hashjoin.log
			echo
			echo "cmd: ${cmd}"
			echo "failed"
			exit 1
