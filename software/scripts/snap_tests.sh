...
if [ $search -eq 1 ]; then
	...
	cmd="./examples/demo_search -C${snap_card} -E 1024	\
	...
	cmd="./examples/demo_search -C${snap_card} -E 2048	\
...
if [ $memcopy -eq 1 ]; then
	...
	cmd="./examples/demo_memcopy -C${snap_card} -X	\
...
if [ $memcopy -eq 1 -a $memcopy_cardram -eq 1 ]; then
	...
	cmd="./examples/demo_memcopy -C${snap_card}		\
...
if [ $hashjoin -eq 1 ]; then
	...
	for t2_entries in `seq 1 128` 512 666 888 999 1024 2048 2049 5015 7007 8088 123123 ; do
		...
		cmd="./examples/demo_hashjoin -C${snap_card} -T ${t2_entries} -v \
